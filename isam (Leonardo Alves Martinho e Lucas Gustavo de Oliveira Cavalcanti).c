//Alunos:
//Leonardo Alves Martinho
//Lucas Gustavo de Oliveira Cavalcanti
...
void* isam_ler_dado_chave_no_interno(T_ISAM *isam, int chave_reg){
    void* dado_func; 
    dado_func = isam->criar_dado(); 
    isam->ler_dado_pos(isam->arq_dados,dado_func,chave_reg); 
    return dado_func; 
}


//Questão B)
//Imprime registros de passagem 
void imprimir_dados_caminho(TFunc *dado_func){
    printf("\nRegistro de passagem");
    printf("\n________________________________________________________________________________________________________________________________________");
    printf("\nCodigo:%d",dado_func->cod);
    printf("\nNome:%s",dado_func->nome);
    printf("\nSalario: %.2f",dado_func->salario);
    printf("\n________________________________________________________________________________________________________________________________________\n");
}

//Imprime nós de passagem
void imprimir_no_caminho(TNo_ISAM *no){
    printf("\nNo de passagem");
    printf("\n________________________________________________________________________________________________________________________________________");
    if(no->tipo == INTERNO) printf("\nTipo de no: INTERNO\n");
    else if(no->tipo == FOLHA) printf("\nTipo de no: FOLHA\n");
    else if(no->tipo == OVERFLOW) printf("\nTipo de no: OVERFLOW");
    int i;
    printf("\nChaves: ");
    for(i = 0; i < no->t-1; i++) printf("%d\t",no->chaves[i]);
    printf("\nFilhos: ");
    for(i = 0; i < no->t; i++) printf("%d\t",no->filhos[i]);
    printf("\n________________________________________________________________________________________________________________________________________\n");
}


//Busca pelo dado no arquivo de dados que satisfaz a consulta (Funciona por Código e por Nome)
void* isam_buscar(T_ISAM *isam, void *consulta){
    void *dummy = (void*) consulta;

    TNo_ISAM *no = isam_criar_no(isam,INTERNO);
    isam_inicializar_no(no);
    isam_ler_no_pos(isam,no, isam->raiz);
    isam_imprimir_no(isam,no);
    imprimir_no_caminho(no);
    ...
    while(no->tipo != FOLHA){
        ...
        for(int i = 0; i < no->n; i++){
            ...
            if(no->chaves[i] == INFINITO) {
                ...
                i = 0;
                dado_func = isam_ler_dado_chave_no_interno(isam, no->chaves[i]);
                isam_imprimir_no(isam,no);
                imprimir_no_caminho(no);
            }

            else dado_func = isam_ler_dado_chave_no_interno(isam, no->chaves[i]);
            imprimir_dados_caminho(dado_func);
            
            if(isam->comparar(dummy,dado_func) == -1){
                isam_ler_no_pos(isam, no, no->filhos[i]);
                isam_imprimir_no(isam,no);
                imprimir_no_caminho(no);
                break;
            ...
            if((isam->comparar(dummy,dado_func) == 0 || isam->comparar(dummy,dado_func) == 1) && (i == isam->t-2)){
    ...
    for(int j = 0; j < isam->t; j++){
        void *dado_resp = isam_ler_dado_chave_no_interno(isam, no->filhos[j]);
        if(isam->comparar(dummy,dado_resp) == 0){ 
            resposta = dado_resp; 
            break; 
        }
    }
...
//Questão C)
//Insere novo registro e atualiza o ISAM
int isam_insere(T_ISAM *isam,void *dado){
    void *novo = (void*) dado;

    void *aux = isam_buscar(isam,novo);
    if(aux!=NULL){
        return 0;
    }else{
        free(aux);
        
        TIndiceReg arq_fim = ++isam->tam_arq_dados;
        TNo_ISAM *no = isam_criar_no(isam,INTERNO);
        isam_ler_no_pos(isam,no,isam->raiz);    
            ...
            while (no->tipo != FOLHA){
                ...
                for (int i = 0; i < isam->t-1; i++){
                    ...
                    if(isam->comparar(dado,dado_func) == -1){
                        isam_ler_no_pos(isam, no, no->filhos[i]);
                        imprimir_no_caminho(no);
                        break;
                    }

                    if((isam->comparar(dado,dado_func) == 0 || isam->comparar(dado,dado_func) == 1) && (i == isam->t-2)){
                        isam_ler_no_pos(isam, no, no->filhos[i+1]);
                        imprimir_no_caminho(no);
                        break;
                    }
        ...
        TIndiceReg novo_ind = isam_pos_arq_ind(isam);
        isam_salvar_no_pos(isam,no,novo_ind);
        return 1;         
    } 
}